...
{
	BW_Read_I2C2(ADDRESS_EEPROM,1,PAGE_TRACER_RULE_UP);
	TCS3200_Transmit(UART5, CMD_TRACER_COPY_DATA);
	TCS3200_Transmit(UART5, (char)DATA);
	BW_Read_I2C2(ADDRESS_EEPROM,1,PAGE_TRACER_RULE_DOWN);
	TCS3200_Transmit(UART5, (char)DATA);
	lcd_display_clear();
	lcd_gotoxy(0,0);sprintf(lcd,"LOAD DONE");lcd_putstr(lcd);
}
